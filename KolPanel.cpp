...
namespace AGZS {
	...
	void TKolPanel::MakeFuel(double f, double c) {
		...
		if (NowFuel >= MaxFuel) {
			this->isFuel = false;
			SetInfo("Заправлено!");
			FMain->StopFuel(this->NowFuel); // isFuelComing = false;
			...
			//isFuelComing = true;
			//if (this->isFuel) {	SetInfo("Идет заправка"); }
			if (isFuel && (f > 0.0)) { // && (c > 0.0)) {
				isFuelComing = true;
				SetInfo("Идет заправка");
			}
		}
	}
	bool TKolPanel::CheckForCalc(double f, double c) {
		if (NowFuel >= MaxFuel) { InfoLabel->Text = "Заправлено!"; return false; }
		InfoLabel->Text = "Остановлено";
		return true;
	}
