...
namespace AGZS {
	...
	private: int FKolNum;
		   ...
		   bool isFuelComing;
	public: bool isFuel;
		  bool isFueling(void) { return isFuelComing; }
		  void setFueling(bool is) { isFuelComing = is; }
	...
	public:
		...
		TKolPanel(int KolNum, double _Cost, MainForm^ MF)
		{
			...
			Cost0Label->Text = String::Format("{0:0.00}", cost);
		...
		// MainForm::OutReceived(): if (have1 || have2) this->StopFuel(); // Two times! Заправлено - Остановлено
		void StopFuel(void); // InfoLabel->Text = "Остановлено"
		void StopFuel(double f);
		void StopFuel(double f, double c);
		bool CheckForCalc(double f, double c);
		...
		void NewCost(double newc) {
			this->cost = newc;
			//Cost0Label->Text = cost.ToString();
			Cost0Label->Text = String::Format("{0:0.00}", newc);
		}
	...
	protected:
		...
		void InitializeComponent(void) {
			...
			MakePanel(&Cost0Label, L"ЦЕНА"); Cost0Label->Text = L"0.00";
			MakePanel(&Cost1Label, L"СУММА"); Cost1Label->Text = L"0.00";
			MakePanel(&FuelLabel, L"ЛИТРЫ"); FuelLabel->Text = L"0.00";
			// Label 2
